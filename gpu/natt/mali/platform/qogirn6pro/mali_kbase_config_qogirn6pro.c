...
void mali_poweroff_second_part(void)
{
	...
	if (atomic_read(&gpu_dvfs_ctx.gpu_power_state) == 0) {
		...
		printk(KERN_ERR "%s failed to disable vddgpu, gpu_power_state = %d, gpu_clock_state = %d, dcdc_gpu_state = %d\n",
				__func__, atomic_read(&gpu_dvfs_ctx.gpu_power_state), atomic_read(&gpu_dvfs_ctx.gpu_clock_state), atomic_read(&gpu_dvfs_ctx.dcdc_gpu_state));
	}
	up(gpu_dvfs_ctx.sem);
}
