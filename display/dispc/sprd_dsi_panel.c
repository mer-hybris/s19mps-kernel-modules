...
int (*tp_proximity_state)(void) = NULL;
void (*export_enable_tp_sleep)(void) = NULL;
void (*export_open_tp_proximity)(void) = NULL;
EXPORT_SYMBOL(tp_proximity_state);
EXPORT_SYMBOL(export_enable_tp_sleep);
EXPORT_SYMBOL(export_open_tp_proximity);
#endif
...
#ifdef PRJ_FEATURE_H_BOARD_INCELL_TP_PROXIMITY_CONTROL
		//printk("%s() qinfan lcd name \n",info->of_node->name);
		if (!(strcmp(info->of_node->name, "lcd_ft8006_mipi"))&&(state == 1)){
			if (export_open_tp_proximity)	
				export_open_tp_proximity();
		}
#endif
	}
...
static int sprd_panel_disable(struct drm_panel *p)
{
	...
	if (!(strcmp(info->of_node->name, "lcd_ft8006_mipi"))&&(state == 0)) {
		if(export_enable_tp_sleep)
	...
	struct sprd_panel *panel)
{
	...
	/* revo.tjd add for power(avdd/avee/...) always on, even in suspend mode*/
	if (of_property_read_bool(lcd_node, "revo,pwr-always-on"))
		info->pwr_always_on = true;
	else
		info->pwr_always_on = false;
...
#ifdef PRJ_FEATURE_H_BOARD_INCELL_TP_PROXIMITY_CONTROL
	p = of_get_property(lcd_node, "sprd,sleep-face_in-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_SLEEP_FACE_IN] = p;
		info->cmds_len[CMD_CODE_SLEEP_FACE_IN] = bytes;
	} else
		DRM_ERROR("can't find sprd,sleep-out-command property\n");
		
	p = of_get_property(lcd_node, "sprd,sleep-face_out-command", &bytes);
	if (p) {
		info->cmds[CMD_CODE_SLEEP_FACE_OUT] = p;
		info->cmds_len[CMD_CODE_SLEEP_FACE_OUT] = bytes;
	} else
		DRM_ERROR("can't find sprd,sleep-out-command property\n");
#endif
...
char secboot_state[10];
const char *str_secboot_state;
#endif
...
static int sprd_panel_parse_dt(struct device_node *np, struct sprd_panel *panel)
{
	...
	if (!rc) {
		...
		if (str_secboot_state) {
			sscanf(str_secboot_state, "secboot=%s", secboot_state);
		}
#endif
	} else {
...
//qzhu add start for store lcd info
struct class *hardware_class;
struct device *hardware_dev;

#if defined(PRJ_FEATURE_H_GET_SECBOOT_STATE)
static ssize_t secboot_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", secboot_state);
}
static ssize_t secboot_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(secboot_state, 0644, secboot_state_show, secboot_state_store);
static void Create_sys_dir(void)
{
  hardware_class = class_create(THIS_MODULE,"lcd_info" );
  if(IS_ERR(hardware_class))
		printk("Failed to create hardware class!\n");

	hardware_dev = device_create(hardware_class,NULL, 0, NULL, "device");
	if(IS_ERR(hardware_dev))
	  printk("Failed to create hardware device!\n");

	if(device_create_file(hardware_dev, &dev_attr_secboot_state) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_secboot_state.attr.name);
}
//qzhu add end
#endif
