...
int dsi_panel_set_dpms_mode(struct sprd_dsi *dsi)
{
	...
	if (!dsi->ctx.enabled) {
		mutex_unlock(&dsi->lock);
		DRM_DEBUG("dsi is not inited,just skip\n");
		return 0;
	}
...
static void sprd_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (!encoder->crtc || !encoder->crtc->state->active ||
		...
		DRM_DEBUG("skip dsi resume\n");
		mutex_unlock(&dsi->lock);
		return;
	}

	if (dsi->ctx.enabled) {
		DRM_DEBUG("dsi is initialized\n");
		mutex_unlock(&dsi->lock);
		return;
	}
...
static void sprd_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	if (!dsi->ctx.enabled) {
		DRM_DEBUG("dsi isn't initialized\n");
		mutex_unlock(&dsi->lock);
		return;
	}
