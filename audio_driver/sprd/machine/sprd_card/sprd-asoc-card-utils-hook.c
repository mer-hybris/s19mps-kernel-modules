...
#ifdef PRJ_FEATURE_H_BOARD_AUDIO_SMARTPA_AW87XXX
static char *aw_profile[] = {"Music", "Off"};/* aw87xxx_acf.bin 文件配置场景 */
...
extern int (*func_aw87xxx_add_codec_controls)(void *codec);
extern int (*func_aw87xxx_set_profile)(int channel, char *profile);
...
retry:
	if(!func_aw87xxx_add_codec_controls || !func_aw87xxx_set_profile)
	{
		pr_info("%s driver not ready yet, cnt = %d", __func__, ++cnt);
		msleep(100);
		if(cnt < 40)
			goto retry;
	}
	
	if(!func_aw87xxx_add_codec_controls) {
		pr_info("%s func_aw87xxx_add_codec_controls: is null, direct return\n", __func__);
		return HOOK_NONE;
	}
	
	if(!func_aw87xxx_set_profile) {
		pr_info("%s func_aw87xxx_set_profile: is null, direct return\n", __func__);
		return HOOK_NONE;
	}

	if(on){
		(*func_aw87xxx_set_profile)(AW_DEV_0, aw_profile[0]);
		(*func_aw87xxx_set_profile)(AW_DEV_1, aw_profile[0]);
		(*func_aw87xxx_set_profile)(AW_DEV_2, aw_profile[0]);
		(*func_aw87xxx_set_profile)(AW_DEV_3, aw_profile[0]);
	}
	else {
		(*func_aw87xxx_set_profile)(AW_DEV_0, aw_profile[1]);
		(*func_aw87xxx_set_profile)(AW_DEV_1, aw_profile[1]);
		(*func_aw87xxx_set_profile)(AW_DEV_2, aw_profile[1]);
		(*func_aw87xxx_set_profile)(AW_DEV_3, aw_profile[1]);
	}
