...
static void hx9031a_ch_cfg(uint8_t chip_select)
{
    ...
    if(HX9023E_ON_BOARD == chip_select) {
        //E823,WIFI:CS0,CS2, Primary:CS1
        CH0_POS = CS0; //WIFI
        CH0_NEG = NA;//装整机后配成CS2; //REF
        CH1_POS = CS1; //Primary ANT
        CH1_NEG = NA;
        CH2_POS = CS2;
        CH2_NEG = NA;
        CH3_POS = NA;
    ...
    for (ii = 0; ii < num_channels; ii++) {
        ...
        if (BODYACTIVE == touch_state) {
            if (channel_p->state == BODYACTIVE)
                ...
                input_report_abs(input, ABS_CODE[ii], 2);
                input_sync(input);
            ...
            if (channel_p->state == PROXACTIVE)
                ...
                input_report_abs(input, ABS_CODE[ii], 1);
                input_sync(input);
            ...
            if (channel_p->state == IDLE)
                ...
                input_report_abs(input, ABS_CODE[ii], 0);
                input_sync(input);
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if (strcmp(sensors_cdev->name, hx9031a_channels[ii].name) == 0) {
            if (1 == enable) {
                ...
                input_report_abs(hx9031a_channels[ii].hx9031a_input_dev, ABS_CODE[ii], 0);
                ...
                input_report_abs(hx9031a_channels[ii].hx9031a_input_dev, ABS_CODE[ii], -1);
                input_sync(hx9031a_channels[ii].hx9031a_input_dev);
            } else {
...
//}end =============================================================
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if ((hx9031a_pdata.channel_used_flag >> ii) & 0x1) { //used flag的值来自dts，表示通道有效性，每一位表示一个通道。
            ...
            input_set_abs_params(hx9031a_channels[ii].hx9031a_input_dev, ABS_CODE[ii], -1, 100, 0, 0);
...
#ifdef CONFIG_PM_WAKELOCKS
	hx9031a_wake_lock = wakeup_source_create("hx9031a_wakelock");
	wakeup_source_add(hx9031a_wake_lock);
#else
...
failed_request_irq:
#ifdef CONFIG_PM_WAKELOCKS
    wakeup_source_destroy(hx9031a_wake_lock);
#else
