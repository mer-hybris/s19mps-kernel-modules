...
#define DEVICE_NAME		"stk3x1x_pls"
...
#define STK3X1X_PLS_MISC_DEVICE 		"stk3x1x_pls" /* for misc_device */
static int stk3x1x_pls_misc_opened=0; /* for misc_device */
static struct stk3x1x_data *stk3x1x_data_misc = NULL; /* for misc_device */
...
static ssize_t stk_ps_distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    //ret = stk3x1x_get_flag(ps_data);
    ret = stk3x1x_i2c_smbus_read_byte_data(ps_data->client, (16));
	if(ret < 0) {
		return ret;
	}
...
static struct device_attribute ps_enable_aso_attribute = __ATTR(enableaso,0644,stk_ps_enable_aso_show,stk_ps_enable_aso_store);
...
static int proximity_gpio_int = 0;
...
static int stk3x1x_pls_misc_open(struct inode *inode, struct file *file)
{
	printk(KERN_INFO "%s\n", __func__);
	if (stk3x1x_pls_misc_opened)
		return -EBUSY;
#ifndef     STK_POLL_PS
    irq_set_irq_type(stk3x1x_data_misc->irq, IRQF_TRIGGER_LOW | IRQF_ONESHOT);
#endif
	stk3x1x_pls_misc_opened = 1;
	return 0;
}

static int stk3x1x_pls_misc_release(struct inode *inode, struct file *file)
{
    printk(KERN_INFO "%s\n", __func__);
	stk3x1x_pls_misc_opened = 0;
    printk(KERN_INFO "Revo_info ps_distance_last=%d \n",stk3x1x_data_misc->ps_distance_last);
  mutex_lock(&stk3x1x_data_misc->io_lock);
#ifndef     STK_POLL_PS
    irq_set_irq_type(stk3x1x_data_misc->irq, IRQF_TRIGGER_FALLING | IRQF_ONESHOT);
#endif
    stk3x1x_enable_ps(stk3x1x_data_misc, 1, 1); /* 0: enable*/
    //stk3x1x_enable_als(stk3x1x_data_misc, 0);
  mutex_unlock(&stk3x1x_data_misc->io_lock);
	
	return 0;
}

static long stk3x1x_pls_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LTR_IOCTL_GET_PFLAG:
		flag = stk3x1x_data_misc->ps_enabled? 1:0;
		break;
		
	case LTR_IOCTL_GET_LFLAG:
		flag = stk3x1x_data_misc->als_enabled? 1:0;
		break;
	...
	case LTR_IOCTL_SET_PFLAG:
		...
		if(flag==1){
		  ...
		  mutex_lock(&stk3x1x_data_misc->io_lock);
            //stk3x1x_enable_ps(stk3x1x_data_misc, 0, 1); /* 0: disable*/
			mutex_unlock(&stk3x1x_data_misc->io_lock);
		}
		break;
	...
	case LTR_IOCTL_SET_LFLAG:
		...
		if(flag==1){
		  mutex_lock(&stk3x1x_data_misc->io_lock);
			stk3x1x_enable_als(stk3x1x_data_misc, 1);   /* 0: enable*/
			mutex_unlock(&stk3x1x_data_misc->io_lock);
		}
		else if(flag==0) {
		  mutex_lock(&stk3x1x_data_misc->io_lock);
			stk3x1x_enable_als(stk3x1x_data_misc, 0);   /* 0: disable*/
			mutex_unlock(&stk3x1x_data_misc->io_lock);
		}
		break;
...
static struct file_operations stk3x1x_pls_miscfops = {
	.owner				= THIS_MODULE,
	.open				= stk3x1x_pls_misc_open,
	.release			= stk3x1x_pls_misc_release,
	.unlocked_ioctl = stk3x1x_pls_misc_ioctl,	
};
static struct miscdevice stk3x1x_pls_miscdevice = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = STK3X1X_PLS_MISC_DEVICE,
	.fops = &stk3x1x_pls_miscfops,
};
...
static int stk3x1x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	stk3x1x_set_wq(ps_data);	/*for poll: PS or ALS*/
  ...
  err = misc_register(&stk3x1x_pls_miscdevice);
  if (err) {
    printk(KERN_ERR "%s: stk3x1x_pls_miscdevice register failed\n", __func__);
		goto err_device_miscregister_failed;
	}

#if (!defined(STK_POLL_ALS) || !defined(STK_POLL_PS))
	enable_irq(ps_data->irq);
	printk("stk3x1x_probe: enable_irq--ps_data->irq\n");
#endif
...
{
	struct stk3x1x_data *ps_data = i2c_get_clientdata(client);
	misc_deregister(&stk3x1x_pls_miscdevice);
	device_init_wakeup(&client->dev, false);
#if (!defined(STK_POLL_ALS) || !defined(STK_POLL_PS))
	free_irq(ps_data->irq, ps_data);
	gpio_free(ps_data->int_pin);	
...
static const struct of_device_id stk3x1x_match_table[] = {
		{.compatible = "stk3x1x,stk3x1x_pls",},
		{ },
};
