					...
					struct jadard_i2c_platform_data *pdata)
{
	...
	printk("DT:MAX_PT = %d, INT_IS_EDGE = %d\n", pjadard_ic_data->JD_MAX_PT,
...
{
	if(!pjadard_ts_data || !pjadard_ts_data->client) {
		pr_info("%s: NULL\n", __func__);
		return;
	}
...
int jadard_common_suspend(struct device *dev)
{
...
int jadard_common_resume(struct device *dev)
{
	struct jadard_ts_data *ts = dev_get_drvdata(dev);
	if(ts) {
...
static int __init jadard_common_init(void)
{
	...
	printk("Jadard common touch panel driver init\n");

#ifndef CONFIG_JD_DB
	ret = i2c_add_driver(&jadard_common_driver);
	pr_info("%s: return %d\n", __func__, ret);
#endif
