...
static uint8_t jd9365t_GetMoveInfoNumber(uint8_t *pFileData)
{
	if (pFileData[JD9365T_HW_HEADER_LEN] == 0xFF) {
		...
		return (uint8_t)((pFileData[JD9365T_HW_HEADER_LEN] - JD9365T_HW_HEADER_INITIAL_NUMBER) /
...
static bool module_jd9365t_chip_detect(void)
{
	...
	if (romid == CHIP_ID) {
		...
		snprintf(pjadard_ic_data->chip_id, sizeof(pjadard_ic_data->chip_id), JD9365T_OUTER_ID);
...
#if defined(MULTI_MODULES) || defined(CONFIG_CHIP_DTCFG) 
...
MODULE_LICENSE("GPL");
#else
void add_module_jd9365t(void)
{
	jadard_jd9365t_probe();
}
void remove_module_jd9365t(void)
{
	jadard_jd9365t_remove();
}
#endif