...
extern  int glb_chsc5x_tp_sysfs_init(struct i2c_client *client);
...
int glb_chsc5x_ts_suspend(void) { return semi_touch_suspend_entry(NULL);}
int glb_chsc5x_ts_resume(void) {return semi_touch_resume_entry(NULL);}

static int int_gpio_no = 0, rst_gpio_no = 0;
...
int semi_touch_get_int(void)
{
    ...
    ret = gpio_request(int_gpio_no, "chsc_int_pin");
    if (ret < 0) {
	    printk("Failed to request reset gpio:%d, err:%d", int_gpio_no, ret);
	    return -1;
    }
...
int semi_touch_get_rst(void)
{
    ...
    ret = gpio_request(rst_gpio_no, "chsc_rst_pin");
    if (ret < 0) {
    	printk("Failed to request irq gpio:%d, err:%d", rst_gpio_no, ret);
    	gpio_free(int_gpio_no);
    	return -1;
    }
...
/********************************************************************************************************************************/
/*#if(!defined(CONFIG_ADF) && !defined(CONFIG_HAS_EARLYSUSPEND) && !defined(CONFIG_DRM))
...
static int semi_touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = glb_chsc5x_tp_sysfs_init(client);
    if (ret) {
        kernel_log_d( "could not create sysfs device attributes , < ts_suspend >");
    }
    check_return_if_fail(ret, NULL);
