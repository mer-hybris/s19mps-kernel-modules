...
static ssize_t tp_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "jdchipset_v0178,IC ID: %s\n", pjadard_ic_data->chip_id);
}

static DEVICE_ATTR_RO(tp_name);
static DEVICE_ATTR_RW(ts_suspend);

static struct attribute *jadard_ts_debug_attrs[] = {
	&dev_attr_tp_name.attr,
...
static int jadard_ts_filesys_create(struct jadard_ts_data *ts)
{
	...
	/* convenient access to sysfs node */
	retval = sysfs_create_link(NULL, &ts->dev->kobj, "board_properties");
	if (retval < 0) {
		JD_E("%s: Failed to create link!\n", __func__);
		return -ENOMEM;
	}
