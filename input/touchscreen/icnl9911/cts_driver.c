...
/*
	Global
*/
extern void chipone_icnt9911_add_tp_name(void *ts_data);
struct chipone_ts_data * g_cts_data = NULL;
int glb_chipone_icnt9911_resume(void)
{
	 struct chipone_ts_data * cts_data = g_cts_data;
	 if(cts_data == NULL )
	 {
		 cts_info("Error NOT set chipone_ts_data  !!!! ");
		 return  -1 ; 
	 }
	 return cts_driver_resume(cts_data);	 
}

int glb_chipone_icnt9911_suspend(void)
{
	 struct chipone_ts_data * cts_data = g_cts_data;
	 if(cts_data == NULL )
	 {
...
int cts_driver_probe(struct device *device, enum cts_bus_type bus_type)
{
    ...
    chipone_icnt9911_add_tp_name(cts_data);
