...
static int get_earjack_state(struct cts_earjack_detect_data *ed_data)
{
    ...
    file = filp_open_block(ed_data->earjack_state_filepath, O_RDONLY, 0);
    if (IS_ERR(file)) {
    ...
    read_size = revo_kernel_read(file, buff, sizeof(buff), &pos);
#else
    read_size = revo_kernel_read(file, pos, buff, sizeof(buff));
#endif
    if (read_size < 0) {
