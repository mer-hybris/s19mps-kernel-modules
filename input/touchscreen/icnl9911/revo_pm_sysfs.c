...
/*
	static
*/
extern int glb_chipone_icnt9911_suspend(void);
extern int glb_chipone_icnt9911_resume(void);

#define  REVO_TS_SUSPEND_FUNC           \
	do { \
		glb_chipone_icnt9911_suspend();\
	}while(0)

#define  REVO_TS_RESUME_FUNC             \
	do { \
		glb_chipone_icnt9911_resume() ;\
	}while(0)
...
/*
	Global
*/
int glb_chipone_icnt9911_tp_sysfs_init(struct i2c_client *client) {
  int err = 0;
...
void glb_chipone_icnt9911_tp_sysfs_remove(struct i2c_client *client) {
