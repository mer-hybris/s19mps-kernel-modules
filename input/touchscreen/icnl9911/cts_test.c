...
int cts_write_file(struct file *filp, void *data, size_t size)
{
    loff_t  pos;
    ssize_t ret;
    ...
    ret = revo_kernel_write(filp, data, size, &pos);
#else
    ret = revo_kernel_write(filp, data, size, pos);
#endif
...
// #else
//         ret = sys_mkdir(dirname, 0777); //tjd remove
// #endif
        ret = -1;
        if (ret < 0 && ret != -EEXIST) {
...
#define START_BANNER \
    ...
    cts_test_data_filp = filp_open_block(filepath,
