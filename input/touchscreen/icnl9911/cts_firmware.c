        ...
        const char *filepath, u16 curr_version)
{
    ...
    file = filp_open_block(filepath, O_RDONLY, 0);
    if (IS_ERR(file)) {
    ...
    read_size = revo_kernel_read(file, buff, 2, &pos);
#else
    read_size = revo_kernel_read(file, pos, buff, 2);
#endif
    if (read_size < 0) {
    ...
    pos = 0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0)
    read_size = revo_kernel_read(file, firmware->data, firmware->size, &pos);
#else
    read_size = revo_kernel_read(file, pos, firmware->data, firmware->size);
#endif
