...
#if defined (REVO_CHIP_TYPE_FT8006AN)
    if (((val[0] == 0x86) && (val[1] == 0x32)) ||
        ((val[0] == 0x86) && (val[1] == 0x42)) ||
        ((val[0] == 0xF0) && (val[1] == 0xC6))) {
        FTS_INFO("tp run in romboot");
        *fw_sts = FTS_RUN_IN_ROM;
    } else if (((val[0] == 0x86) && (val[1] == 0xC2)) ||
               ((val[0] == 0x86) && (val[1] == 0xE2)) ||
               ((val[0] == 0xF0) && (val[1] == 0xD6))) {
...
static int fts_read_file(char *file_name, u8 **file_buf)
{
    ...
    filp = revo_filp_open(file_path, O_RDONLY, 0);
    if (IS_ERR(filp)) {
    ...
    *file_buf = (u8 *)vmalloc(file_len);
    ...
    set_fs(KERNEL_DS);
    pos = 0;
	ret = revo_kernel_read(filp, *file_buf, file_len , &pos);
    if (ret < 0)
