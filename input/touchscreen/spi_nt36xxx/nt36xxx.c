 ...
 *
 * $Revision: 106431 $
 * $Date: 2022-10-04 09:03:03 +0800 (Tue, 04 Oct 2022) $
 *
...
extern int glb_novatek_tp_sysfs_init(struct spi_device *client);
...
#if defined(REVO_PM_TP_SYSFS)
	ret = glb_novatek_tp_sysfs_init(client);
	if (ret < 0)
	{
		NVT_ERR("Add powermanger sysfs  failed. ret=%d\n", ret);
		goto err_revo_pm_tp_sysfs_failed;
	}
...
int32_t glb_novatek_revo_nvt_ts_suspend(void)
{
	if(ts)
		nvt_ts_suspend(&ts->client->dev);
	else
		printk("ts is null , please chk!!");
	return  0 ;
}

int32_t glb_novatek_revo_nvt_ts_resume(void)
{
	if(ts)
