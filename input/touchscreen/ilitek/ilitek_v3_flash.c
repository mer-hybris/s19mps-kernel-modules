...
int ili_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu)
{
	...
	f = revo_filp_open(DUMP_IRAM_PATH, O_WRONLY | O_CREAT | O_TRUNC, 644);
	if (ERR_ALLOC_MEM(f)) {
	...
	pos = 0;
	revo_kernel_write(f, fw_buf, len, &pos);
	filp_close(f, NULL);
...
int ili_fw_dump_flash_data(u32 start, u32 end, bool user, bool mcu)
{
	...
	f = revo_filp_open(DUMP_FLASH_PATH, O_WRONLY | O_CREAT | O_TRUNC, 644);
	if (ERR_ALLOC_MEM(f)) {
		ILI_ERR("Failed to open the file at %ld.\n", PTR_ERR(f));
		ret = -1;
		goto out;
	...
	pos = 0;
	revo_kernel_write(f, buf, length, &pos);
	filp_close(f, NULL);
	ipio_vfree((void **)&buf);
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case REQUEST_FIRMWARE:
		...
		break;
	case FILP_OPEN:
		f = revo_filp_open(ilits->md_fw_filp_path, O_RDONLY, 0644);
		if (ERR_ALLOC_MEM(f)) {
			ILI_ERR("Failed to open the file at %ld\n", PTR_ERR(f));
			ret = -1;
			goto out;
		...
		pos = 0;
		revo_kernel_read(f, (u8 *)ilits->tp_fw.data, fsize, &pos);
		filp_close(f, NULL);
		ilits->tp_fw.size = fsize;
		break;
	default:
