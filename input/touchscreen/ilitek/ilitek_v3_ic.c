...
int ili_ic_get_panel_info(void)
{
	...
	if (buf[0] != cmd) {
		...
		int ret = revo_get_tp_size(&width,&height);
		ILI_INFO("Invalid panel info, use default resolution ret:%d\n", ret);
		ilits->panel_wid = width;
		ilits->panel_hei = height;
		ilits->trans_xy = OFF;
	} else {
