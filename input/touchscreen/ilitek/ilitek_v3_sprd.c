...
void ili_input_register(void)
{
	...
	ilits->input->name = "currency_tp";// ilits->hwif->name;
...
static void ilitek_plat_gpio_unregister(void)
{
	...
	if (!strcmp(psy->desc->name, "usb")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret < 0) {
				...
				ILI_INFO("usb_plug_status = %d, usb prop.intval =%d\n", ilits->usb_plug_status, prop.intval);
...
static void ilitek_update_charger(struct work_struct *work)
{
	int ret = 0;
	mutex_lock(&ilits->touch_mutex);
	ILI_INFO("tjd:usb_plug_status:%d\n", ilits->usb_plug_status);
...
static int ilitek_plat_notifier_vbus(struct notifier_block *nb,
				unsigned long event, void *data)
{
	ILI_INFO("ilitek_plat_notifier_vbus's event = %ld\n", event);
	ilits->usb_plug_status = event;
	if(!ilits->tp_suspend && (ilits->charger_notify_wq != NULL))
		queue_work(ilits->charger_notify_wq,&ilits->update_charger);
...
void ilitek_plat_charger_init(void)
{
	int ret = 0;
	struct device_node *dev_node = ilits->dev->of_node;
#if 0 //revo tjd add for usb plug event
	/* get vbus/id gpios extcon device */
	if (dev_node && of_property_read_bool(dev_node, "extcon")) {
		ilits->edev = extcon_get_edev_by_phandle(ilits->dev, 0);
		if (!IS_ERR(ilits->edev)) {
			ilits->vbus_nb.notifier_call = ilitek_plat_notifier_vbus;
			ret = extcon_register_notifier(ilits->edev, EXTCON_USB,
							&ilits->vbus_nb);
		} else {
			ret = PTR_ERR(ilits->edev);
			dev_err(ilits->dev, "failed to find vbus extcon device.\n");
		}
	}
	ilits->usb_plug_status = 0;
#else
...
static void ilitek_ts_virtual_keys_init(void)
{
    int ret;
...
static int ilitek_plat_probe(void)
{
	...
	if (ili_tddi_init() < 0) {
		...
		ili_irq_unregister();
		ilitek_plat_gpio_unregister();
		return -ENODEV;
	}
	ilitek_ts_virtual_keys_init();
#if SPRD_SYSFS_SUSPEND_RESUME
...
{
	ILI_INFO("ILITEK TP driver init for SPRD\n");
