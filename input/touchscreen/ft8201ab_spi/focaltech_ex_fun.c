...
extern void revo_fts_resume(void);
extern void revo_fts_suspend(void);
...
static ssize_t tp_name_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if(fts_data)
        return sprintf(buf, "ft8201ab_spi chip id = 0x%02x%02x(cid type=0x%x)",
             fts_data->ic_info.ids.chip_idh, fts_data->ic_info.ids.chip_idl,
             fts_data->ic_info.cid.type);
    else
        return sprintf(buf, "ft8201ab_spi\n");
}
...
static void revo_tp_sysfs_init(void)
{
    int ret = 0;
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
    ...
    if (ret < 0) {
        FTS_ERROR("[EX]: Failed to create link!");
        return -ENOMEM;
    }
	revo_tp_sysfs_init();
    return ret;
