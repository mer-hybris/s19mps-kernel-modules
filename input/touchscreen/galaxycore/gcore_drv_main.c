...
static DEVICE_ATTR(enable, 0664, proximity_enable_show, NULL);
	...
	if(gdev->PS_Enale == true){
		...
		if(coor_data[62] & 0x01){
			...
			if(coor_data[62]&0x80){
				...
				s_nearfar = 0;
				sensorhub_prox_do->prox_reprot_func(gdev->input_device,SENSORHUB_FACE_STATUS_NEAR);
				...
				s_nearfar = 1;
				sensorhub_prox_do->prox_reprot_func(gdev->input_device,SENSORHUB_FACE_STATUS_FAR);
				//input_sync(gdev->input_device);
			}
		}
	}
...
static int tpd_irq_registration(struct gcore_dev *gdev)
{
	...
	ret = devm_request_threaded_irq(&gdev->bus_device->dev, gdev->touch_irq,
					...
					tpd_event_handler,
					IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_SUSPEND, "TOUCH_PANEL-eint", gdev);
	if (ret > 0) {
		ret = -1;
...
{
    #if defined(SENSORHUB_PROXIMITY_TOUCHSCREEN) && defined(PRJ_FEATURE_H_BOARD_INCELL_TP_PROXIMITY_CONTROL)
...
#if defined(SENSORHUB_PROXIMITY_TOUCHSCREEN)	
	set_bit(EV_ABS, gdev->input_device->evbit);
    input_set_capability(gdev->input_device, EV_ABS, ABS_DISTANCE);
    input_set_abs_params(gdev->input_device, ABS_DISTANCE, 0, 1, 0, 0);
    sensorhub_info.input = gdev->input_device;
