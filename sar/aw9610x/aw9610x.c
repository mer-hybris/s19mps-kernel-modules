...
static int32_t aw9610x_power_init(struct aw9610x *aw9610x)
{
	...
	aw9610x->vcc = devm_regulator_get_optional(aw9610x->dev, "vcc");
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_link(NULL, &i2c->dev.kobj, "sar_sensor");
	if (ret < 0) {
		AWLOGE(aw9610x->dev, "Failed to create link!");
		goto err_sysfs;
	}
...
static int aw9610x_suspend(struct device *dev)
{
	...
	disable_irq(aw9610x->to_irq);
	aw9610x_i2c_read(aw9610x, REG_HOSTIRQSRC, &reg_data);//to make sure the irq flag be cleared
	aw9610x_i2c_write(aw9610x, REG_CMD, AW9610X_SLEEP_MODE);
